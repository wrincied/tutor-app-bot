...
> **Не используй боевой `TELEGRAM_BOT_TOKEN` в polling на ПК.** Пока локальный polling крутится, прод на Cloud Run не получает апдейты — кажется, что бот «жив только когда комп включён». Для локалки — отдельный тестовый бот или выключи локальный процесс.
>
> Авто-перевод уроков в «проведён» (+30 мин) делает **backend** (App Hosting), не бот. Если backend запущен только через `npm start` на компе — воркер тоже только пока комп включён. Нужен задеплоенный backend (`minInstances: 1` в `apphosting.yaml`).
